...
import time
import webbrowser
from scraper.orange_scraper import ORANGE_Scraper

color = ["#efefe9", "#e4e4dd"]


def ORANGE_GUI(canvas):
    frame = tk.Frame(canvas)
    frame.pack()
    scraper = ORANGE_Scraper()
    ...
    def open_url(event=None):
        webbrowser.open_new('https://www.accessdata.fda.gov/scripts/cder/ob/default.cfm')
    ...
    labelframe.pack()
    label_list_9(labelframe, 'Ingredient', 'DF', 'Route', 'Trade Name', 'Applicant', 0,
               "black", ('Times', 10, 'bold'))

    for i, row in enumerate(display_data):
        label_list_9(labelframe, row[0], row[1], row[2], row[3], row[4], i + 1)

    print('ORANGE GUI is made!')
    # frame.update_idletasks()
    return frame

def label_list_9(labelframe, data1, data2, data3, data4, data5, ind,
